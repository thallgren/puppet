...
describe Puppet::Pops::Parser::Locator do
  ...
  it "multi byte characters in a comment does not interfere with AST node text extraction" do
    ...
    # jlexer uses a character based locator
    expect(model.body.offset).to eq(Puppet.features.jlexer? ? 11 : 12)
  ...
  it "multi byte characters in a comment does not interfere with AST node text extraction" do
    ...
    # jlexer uses a character based locator
    expect(model.body.offset).to eq(Puppet.features.jlexer? ? 13 : 14) # The '+'
    expect(model.body.length).to eq(1)
    expect(model.body.locator.extract_tree_text(model.body)).to eq('1 + 2')
  end

  it 'Locator caches last offset / line', :unless => Puppet.features.jlexer? do
  ...
  it 'Locator invalidates last offset / line cache if asked for different offset', :unless => Puppet.features.jlexer? do
    #Puppet::Pops::Parser::Locator::AbstractLocator.expects(:ary_bsearch_i).once
    parser = Puppet::Pops::Parser::Parser.new()
    model = parser.parse_string("$a\n = 1\n + 1\n").model
